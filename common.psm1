...
function New-NodeJSEnvPem
{
    ...
    (
        ...
        # Location of cert, e.g., Cert:\LocalMachine\root
        [Parameter(Mandatory = $true)]
        [string] $CertLocation,
    ...
    $rootcerts = Get-ChildItem $CertLocation | Where-Object Subject -eq "CN=${CertName}"
    if (-not $rootcerts) {
    ...
    $bundledPem = [System.IO.Path]::Combine($PemFolder,"AzureStackBundlePem.pem")
    # Remove the previous bundled pem if it already exists.
    if (Test-Path -Path $bundledPem)
    {
        Remove-Item -Path $bundledPem -Force
    }
