...
$global:commonScript = [io.path]::combine($PSScriptRoot, "common.psm1")
...
switch ($Type)
{
    ...
    {
        $pesterVersion = "5.2.1"
        $pesterModuleInfo = Get-Module -ListAvailable | Where-Object {$_.Name -eq "pester" -and $_.Version -eq $pesterVersion}
        if (!$pesterModuleInfo)
        {
        ...
        {
            # Remove existing imported Pester module to prevent conflict in case an older version was already imported.
